...
public class AgentTool implements Tool {
    ...
    private com.lingcode.worktree.WorktreeManager worktreeManager;

    /** Optional: team manager for team_name registration. */
    private com.lingcode.teams.TeamManager teamManager;
    ...
    public void setWorktreeManager(com.lingcode.worktree.WorktreeManager worktreeManager) {
        this.worktreeManager = worktreeManager;
    }

    public void setTeamManager(com.lingcode.teams.TeamManager teamManager) {
        ...
        for (var msg : parent.getMessages()) {
            if (msg.getToolUses() != null && !msg.getToolUses().isEmpty()
                ...
                var placeholders = msg.getToolUses().stream()
                        .map(tu -> new com.lingcode.conversation.ToolResultBlock(
    ...
    private ToolResult runSync(SubAgentSpec spec, String description, String prompt, String modelOverride, String isolation) {
        ...
        com.lingcode.worktree.AgentWorktree.Result wtResult = null;
        if ("worktree".equals(isolation) && worktreeManager != null) {
            ...
            try {
                wtResult = com.lingcode.worktree.AgentWorktree.create(
                ...
                String notice = com.lingcode.worktree.AgentWorktree.buildNotice(
        ...
        while (true) {
            ...
            switch (event) {
                ...
                case AgentEvent.LoopComplete lc -> {
                    ...
                    String wtInfo = "";
                    if (wtResult != null) {
                        if (com.lingcode.worktree.WorktreeChanges.hasChanges(
                            ...
                            com.lingcode.worktree.AgentWorktree.remove(
    ...
    private ToolResult runAsTeammate(SubAgentSpec spec, String teamName,
        ...
        if (team == null) {
            team = teamManager.createTeam(teamName, com.lingcode.teams.TeamManager.detectBackend(),
                    com.lingcode.teams.TeammateRunner.LEAD_NAME, description);
        }
        ...
        ToolRegistry subRegistry = ToolFilter.filterForTeammate(parentRegistry, spec);
        // Add coordination tools for teammates
        subRegistry.register(new com.lingcode.teams.TeamTools.SendMessageTool(teamManager, memberName));
        // 团队共享任务板工具：队友之间共享同一份任务列表（覆盖个人版同名工具）
        subRegistry.register(new com.lingcode.teams.TeamTaskTools.TaskCreateTool(teamManager, teamName, memberName));
        subRegistry.register(new com.lingcode.teams.TeamTaskTools.TaskGetTool(teamManager, teamName));
        subRegistry.register(new com.lingcode.teams.TeamTaskTools.TaskListTool(teamManager, teamName));
        subRegistry.register(new com.lingcode.teams.TeamTaskTools.TaskUpdateTool(teamManager, teamName));
        ...
        // Build addendum
        String addendum = com.lingcode.teams.TeammateRunner.buildTeammateAddendum(
        ...
        if ("worktree".equals(isolation) && worktreeManager != null) {
            try {
                ...
                new java.security.SecureRandom().nextBytes(rndBytes);
                String slug = "agent-a" + java.util.HexFormat.of().formatHex(rndBytes).substring(0, 7);
                var wtResult = com.lingcode.worktree.AgentWorktree.create(
                        slug, worktreeManager.getProjectRoot(), worktreeManager.getSymlinkDirs());
        ...
        // Spawn teammate
        try {
            var spawnResult = com.lingcode.teams.SpawnDispatcher.spawnTeammate(
                    new com.lingcode.teams.SpawnDispatcher.SpawnConfig(
     ...
     */
    private com.lingcode.permission.PermissionChecker teammateChecker(boolean planModeRequired) {
        if (!planModeRequired) {
            // 普通队友不带检查器，沿用既有行为
            return null;
        }
        var root = java.nio.file.Path.of(System.getProperty("user.dir"));
        return new com.lingcode.permission.PermissionChecker(
                com.lingcode.permission.PermissionMode.PLAN, root);
    }
