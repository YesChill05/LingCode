package com.lingcode.agent;

import com.lingcode.config.ProviderConfig;
import com.lingcode.conversation.ConversationManager;
import com.lingcode.conversation.ThinkingBlock;
import com.lingcode.conversation.ToolResultBlock;
import com.lingcode.conversation.ToolUseBlock;
import com.lingcode.hook.HookEngine;
import com.lingcode.llm.LlmClient;
import com.lingcode.llm.StreamEvent;
...
import com.lingcode.prompt.CoordinatorPrompt;
import com.lingcode.prompt.PlanModePrompt;
import com.lingcode.tool.ToolRegistry;
import com.lingcode.toolresult.ToolResultBudget;
...
public class Agent {
    ...
    private final com.lingcode.compact.ContextCompactor.AutoCompactTrackingState compactTracking =
            new com.lingcode.compact.ContextCompactor.AutoCompactTrackingState();
     ...
     */
    private final com.lingcode.compact.RecoveryState recoveryState =
            new com.lingcode.compact.RecoveryState();

    public com.lingcode.compact.RecoveryState getRecoveryState() { return recoveryState; }

    private com.lingcode.filehistory.FileHistory fileHistory;
    public void setFileHistory(com.lingcode.filehistory.FileHistory fh) { this.fileHistory = fh; }
    public com.lingcode.filehistory.FileHistory getFileHistory() { return fileHistory; }
    ...
    private void agentLoop(ConversationManager conv, BlockingQueue<AgentEvent> queue) {
        ...
        for (int iteration = 1; ; iteration++) {
            ...
            try {
                ...
                String compactMsg = com.lingcode.compact.ContextCompactor.manage(
                ...
                if (lastErr != null && (lastErr.contains("context") || lastErr.contains("too long")
                    ...
                    if (contextRetries < 3) {
                        ...
                        try {
                            ...
                            com.lingcode.compact.ContextCompactor.forceCompact(
            ...
            for (var r : results) {
                ...
                if (content.length() > com.lingcode.tool.ToolRegistry.MAX_OUTPUT_CHARS
    ...
    private void persistLastMessage(ConversationManager conv) {
        ...
        com.lingcode.session.SessionManager.saveConversationMessage(
