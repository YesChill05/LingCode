 ...
 * <p>存储结构：
 * <ul>
 *   <li>用户级 (~/.lingcode/memory/)：存放 type=user / type=feedback 的记忆文件</li>
 *   <li>项目级 (.lingcode/memory/)：存放 type=project / type=reference 的记忆文件</li>
 * </ul>
 *
...
public class MemoryManager {
    ...
    private static final String MEMORY_DIR = ".lingcode/memory";
    ...
    /** 返回用户级记忆目录（~/.lingcode/memory/） */
    public Path userMemDir() {
        return userMemDirPath;
    }

    /** 返回项目级记忆目录（.lingcode/memory/） */
    ...
    /**
     * 加载指令文件：支持用户级（~/.lingcode/LINGCODE.md）、项目级（git root 到 workDir 逐层）、
     * 兼容旧版 INSTRUCTIONS.md、私有 LINGCODE.local.md，以及 @include 递归展开。
