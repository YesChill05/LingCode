 ...
 * ({@code ~/.lingcode/skills/}) → project ({@code .lingcode/skills/}),
...
public class SkillCatalog {
    ...
    /**
     * 生成系统提示词里的 Skill 清单：只列名字和一句话描述，完整 SOP 等模型调
     * LoadSkill 时再取。目录为空时返回空串，调用方据此跳过这一段。
     */
    public String buildSection(String workDir) {
        var metas = list();
        if (metas.isEmpty()) return "";
        var skillsDir = Path.of(workDir, ".lingcode", "skills");
    ...
    public static SkillCatalog loadCatalog(String workDir) {
        ...
        if (home != null) {
            c.loadTier(Path.of(home, ".lingcode", "skills"), "user");
        }

        // Tier 3: project
        c.loadTier(Path.of(workDir, ".lingcode", "skills"), "project");
    ...
    private static List<String> skillDirPaths(String workDir) {
        ...
        String home = System.getProperty("user.home");
        if (home != null) {
            dirs.add(Path.of(home, ".lingcode", "skills").toString());
        }
        if (workDir != null) {
            dirs.add(Path.of(workDir, ".lingcode", "skills").toString());
        }
        return dirs;
    }
