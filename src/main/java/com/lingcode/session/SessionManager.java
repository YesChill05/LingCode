...
public class SessionManager {
    ...
    private static Path sessionsDir(String workDir) {
        return Path.of(workDir, ".lingcode", "sessions");
    }
    ...
    public static void saveConversationMessage(String workDir, String sessionId,
                                               com.lingcode.conversation.Message msg) {
        saveRecord(workDir, sessionId, msg.getRole(), null, msg.getContent(),
                toolUseRecords(msg), toolResultRecords(msg));
    }

    private static List<ToolUseRecord> toolUseRecords(com.lingcode.conversation.Message msg) {
    ...
    private static List<ToolResultRecord> toolResultRecords(com.lingcode.conversation.Message msg) {
    ...
    private static ConversationManager replay(List<SessionMessage> messages) {
        ...
        for (SessionMessage msg : messages) {
            ...
            com.lingcode.conversation.Message restored = toConversationMessage(msg);
    ...
    public static com.lingcode.conversation.Message toConversationMessage(SessionMessage msg) {
        List<com.lingcode.conversation.ToolUseBlock> uses = new ArrayList<>();
        for (ToolUseRecord tu : msg.toolUses()) {
            uses.add(new com.lingcode.conversation.ToolUseBlock(
                    tu.toolUseId(), tu.toolName(), tu.arguments()));
        }
        List<com.lingcode.conversation.ToolResultBlock> results = new ArrayList<>();
        for (ToolResultRecord tr : msg.toolResults()) {
            results.add(new com.lingcode.conversation.ToolResultBlock(
                    tr.toolUseId(), tr.content(), tr.isError()));
        }
        var restored = new com.lingcode.conversation.Message(msg.role(), msg.content());
    ...
    public static KeepMessage toKeepMessage(com.lingcode.conversation.Message msg) {
